...
function CompleteProfile({ showHeader = true }: { showHeader?: boolean }) {
  ...
  const isSearching = new URLSearchParams(search).get('search');
  ...
  return (
    <div className="bg-main ">
      {showHeader && (
        <CompleteProfileHeader isSearching={isSearching == null ? false : true} />
      )}
      {user?.data.data.first_name &&
      user?.data.data.email &&
      user?.data.data.last_name &&
      user?.data.data.person?.nid == null ? (
